...
const Skillset = () => {
  ...
  const rowData = async (id) => {
    try {
      let res = await axios.get(`${SERVICE.SKILLSET_SINGLE}/${id}`, {
        // headers: {
  ...
  const filteredData = skillsets.filter((item) =>
  ...
  let updatelist = [];
  updatelist = skillsetedit?.updatedby;
  updatelist?.map((item) => {
  ...
  // Excel
  const fileName = "Skillset";

  const [branchData, setBranchData] = useState([]);
  let excelno = 1;
  // get particular columns for export excel
  const getexcelDatas = async () => {
    let response = await axios.get(SERVICE.SKILLSET, {
      //  headers: {
      //      'Authorization': `Bearer ${auth.APIToken}`
      //  },
    });
    var data = response.data.skillsets.map(t => ({
  ...
  console.log(skillsetedit.updatedby, 'updatedvalue');
  ...
  let skillsetLength = skillsets.length;
  ...
  const selectPageHandler = (selPage) => {
    if (selPage >= 1 && selPage <= (Math.ceil(skillsets.length / (Number(entries)))) && selPage !== pages)
      setPages(selPage)
  ...
  return (
    <Box>
      ...
      <Box sx={userStyle.container}>
        <Typography sx={userStyle.SubHeaderText}>Create Skillset</Typography>
        <>
          <br /> <br />
            ...
            <Grid item md={6} sm={12} sx={12}>
              <FormControl fullWidth size="small" >
                ...
                <OutlinedInput
                  ...
                  type="text"
                  value={skillset.name}
                  onChange={(e) => { setSkillset({ ...skillset, name: e.target.value }) }}
                />
              </FormControl>
            </Grid>
          </Grid>
      ...
      <>
        <Box sx={userStyle.container}>
            ...
            <Box>
              <FormControl size="small" fullWidth>
                ...
                >
                  ...
                  <MenuItem value={(skillsets.length)}>All</MenuItem>
                </Select>
              </FormControl>
            </Box>
            <Box>
          ...
          <TableContainer component={Paper}>
            ...
            >
              ...
              <TableBody align="left">
                {filteredData &&
                  (filteredData.slice((pages * entries - entries < skillsetLength ? pages * entries - entries : 0),
                    ((pages * entries - entries <= skillsetLength ? pages * entries - entries : 0) == 0 ? entries : pages * entries)).map((row, index) => (
          ...
          <Grid style={userStyle.dataTablestyle}>
                        <Box >
                            <Typography>Showing
                                {(pages * entries - entries <= skillsetLength ? (pages * entries - entries + 1) : 0) == 0 ? Math.floor((pages * entries - entries + 1) / (pages * entries - entries)) : (pages * entries - entries + 1)}
                                {/* {(pages * entries - entries + 1)}  */}
                                to {(pages * entries) > skillsets.length ? skillsets.length : ((pages * entries))} of {skillsets.length} entries</Typography>  </Box>
                        <Box>
                            {skillsets && <Typography className="Pagination">
                                <Button sx={userStyle.paginationbtn} onClick={() => { selectPageHandler(pages - 1) }}>prev</Button>

                                {[...Array(Math.ceil(skillsets.length / Number(entries)))].map((_, i) => {
                                    if (entries == 1) {
                                        if (i <= 5) {
                ...
                <Grid item md={12} sm={12} sx={12}>
                  <FormControl fullWidth size="small" >
                    ...
                    <OutlinedInput
                      ...
                      onChange={(e) => { setSkillsetedit({ ...skillsetedit, name: e.target.value }) }}
                    />
                  </FormControl><br /><br />
                </Grid>
                <Grid container>
